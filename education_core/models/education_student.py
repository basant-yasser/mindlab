...
class EducationStudent(models.Model):
    ...
    def create(self, vals):
        """Override create to assign sequence for newly created records"""

        # Support single or multiple record creation
        vals_list = vals if isinstance(vals, list) else [vals]

        for v in vals_list:
            v['ad_no'] = self.env['ir.sequence'].next_by_code(
                'education.student'
            )

        return super(EducationStudent, self).create(vals)
